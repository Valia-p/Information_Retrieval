...
def themes_overview():
    ...
    try:
        ...
        # Build reverse map from speech_id -> row index in tfidf
...
def themes_cluster():
    ...
    try:
        ...
        # Build row index: speech_id -> tfidf row
        row_index = {int(sid): i for i, sid in enumerate(doc_ids.tolist())}

        # IMPORTANT: get vocabulary in the SAME order as TF-IDF columns
        # build_tfidf_matrix() used: sorted(DISTINCT keyword)
        conn = sqlite3.connect(DB_NAME)
        cur = conn.cursor()
        cur.execute("SELECT DISTINCT keyword FROM speech_keywords")
        terms = sorted([r[0] for r in cur.fetchall()])

        # --- Collect speeches of this cluster from main DB ---
...
def themes_embedding2d():
    """
    Επιστρέφει 2D embedding (PCA) των LSI vectors, downsampled ανά cluster,
    για να σχεδιαστεί scatter στο UI. Απαιτεί:
      - lsi_projected_docs.npz (LSI_OUTPUT_FILE)
      - doc_ids.npy (DOC_IDS_FILE)
      - final_clustering_results.pkl (CLUSTERS_FILE)
    """
    import numpy as _np, pickle as _pickle

    try:
        # Φόρτωση LSI vectors & αντιστοίχιση σε speech ids
        data = _np.load(LSI_OUTPUT_FILE)["data"]           # (N_docs, K)
        doc_ids = _np.load(DOC_IDS_FILE)                   # (N_docs, )
        with open(CLUSTERS_FILE, "rb") as f:
            clusters = _pickle.load(f)                     # {cluster_id: [speech_ids]}

        # Map: speech_id -> row index
        row_index = {int(sid): i for i, sid in enumerate(doc_ids.tolist())}

        # PCA 2D (χωρίς sklearn, με SVD)
        X = data.astype("float32")
        Xc = X - X.mean(axis=0, keepdims=True)
        # Vt: κύριες κατευθύνσεις, παίρνουμε τα 2 πρώτα components
        _, _, Vt = _np.linalg.svd(Xc, full_matrices=False)
        W2 = Vt[:2].T
        coords = Xc @ W2                                  # (N_docs, 2)

        # Downsample per cluster για ελαφρύ payload
        MAX_PER_CLUSTER = int(request.args.get("per_cluster", 60))
        series = []
        for cid, doc_list in clusters.items():
            # κράτα μόνο όσα υπάρχουν στο row_index
